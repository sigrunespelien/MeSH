...
# dh4096.pem will be used in mesh, but create it here to avoid openssl dependency in mesh image
RUN openssl dhparam -out /etc/ssl/dh4096.pem 4096

# Add and compile libwt from source
WORKDIR /projects
...
# Clone and compile mesh from source
WORKDIR /projects
...
# Final image for importing MeSH data to elasticsearch
...
# Final image for MeSH web application
FROM bitnami/minideb:bookworm AS mesh

# This is most recent boost version in bookworm
...
# Copy libwt to /usr/local/lib and make it part of ld search path
...
RUN useradd --no-create-home --user-group mesh
USER mesh
...
# Add application files from builder image
